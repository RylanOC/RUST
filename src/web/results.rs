...
pub async fn results(
    ...
    for i in 1..6 {
        ...
        tracks.combine(&mut PersonalizationData::Tracks
            .get_data::<TracksVec>(&tokens, &track_params)
            .await
            .map_err(|e| {
                error!("Could not get track data: {}", e);
    ...
    let mut charts = format!("<div class=\"chart-genre\">\n{}\n</div>\n<div class=\"chart-grid\">\n", chart_data.pop().unwrap()).to_string();

    for (i, chart) in chart_data.iter().enumerate() {
        let column = (i%2)+1;
        let row = (i/2)+1;
        let item = format!("<div class=\"chart-item\" style=\"grid-column: {}; grid-row: {};\">\n{}\n</div>\n\n",
            column, row, chart);
    ...
    let webpage = ResultsPage::new(artists, tracks, charts.to_string()).render(hbs_reg).unwrap();
